# -*- coding: utf-8 -*-
# ======================================================
# Boshy Randomizer – GUI (CustomTkinter) v1.3
# Layout:
#   Left  : Stage Selection → Target Collect Mode → Difficulty
#   Right : Item Randomizer → Random Character
# Rules:
#   - Target Mode hides Stage Selection and shows Target slider
#   - In Target Mode: all Levels in Optional tab are checked+disabled
#   - In Target Mode: Gastly (boss5.ini) and Cheetahman (boss21.ini) checked+disabled
#   - Item Randomizer stays selectable even in Target Mode
#   - Sliders show current value labels and clear disabled visuals
#   - Optional tab uses "Checked = included" (unchecked = disabled set)
#   - Taskbar icon from base/Custom/rando_icon.ico
# ======================================================
...
# ======================================================
# Utility: load counts and max items
# ======================================================
...
# ======================================================
# Windows Taskbar Icon (Custom\rando_icon.ico)
# ======================================================
def _try_set_icon(root):
    if platform.system().lower() != "windows":
        return
    try:
        base_dir = os.path.dirname(os.path.dirname(__file__))  # base folder
...
def build_gui_config():
    ...
    # Root
    root = ctk.CTk()
    root.title("Boshy Randomizer – v1.3")
    ...
    # ======================================================
    # LEFT SIDE: Stage Selection → Target Collect → Difficulty
    # ======================================================
    ...
    # --- Target Item Count (initially hidden) ---
    ...
    # --- Hint frame (shown only in Target Mode) ---
    ...
    def apply_target_rules():
        ...
        if on:
            ...
            # Show Target Item Count frame + hint
    ...
    # ======================================================
    # RIGHT SIDE: Item Randomizer → Random Character
    # ======================================================

    # --- Difficulty (moved to right side, above Item Randomizer) ---
    ...
    # ======================================================
    # OPTIONAL TAB (scrollable, two columns)
    # ======================================================
    ...
    # ======================================================
    # Helpers (enable/disable visuals)
    # ======================================================
    ...
    def apply_only_rules():
        ...
        if ob and oroom:
            # Resolve conflict: turn off "Only Levels" if both checked
    ...
    def _force_bosses_gastly_cheetah(force_on: bool):
        for force_name in ("boss5.ini", "boss21.ini"):
            chk = boss_checks.get(force_name)
            if not chk:
                continue
            try:
                if force_on:
                    boss_vars[force_name].set(True)
                    chk.configure(state="disabled")
                else:
                    chk.configure(state="normal")
            except Exception:
    ...
    def on_start():
        ...
        # Difficulty sofort in den Save schreiben
        write_selected_difficulty_to_encrypted_save(cfg.difficulty)

        # Versuche, "its-boshy-time.wav" abzuspielen und das Fenster
        # erst nach 1 Sekunde zu schließen
        try:
            base_dir = os.path.dirname(os.path.dirname(__file__))  # Projekt-Root
            wav_path = os.path.join(base_dir, "Custom", "its-boshy-time.wav")
            if os.path.exists(wav_path):
                try:
                    import winsound
                    # Asynchron abspielen, damit die GUI nicht blockiert
                    winsound.PlaySound(wav_path, winsound.SND_FILENAME | winsound.SND_ASYNC)
                except Exception:
                    # Wenn Sound fehlschlägt, egal – nur kein Crash
                    pass

                # Button deaktivieren, damit man nicht mehrfach klickt
                start_btn.configure(state="disabled", text="Creating Route...")

                # Fenster nach 1000ms schließen
                root.after(1500, root.destroy)
                return
        except Exception:
            # Fallback: falls irgendwas mit Pfaden / winsound schiefgeht
            pass

        # Fallback: sofort schließen wie früher
        root.destroy()

    # ✅ WICHTIG: außerhalb der Funktion
