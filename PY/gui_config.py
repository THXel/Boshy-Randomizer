...
def build_gui_config():
    ...
    def _set_item_randomizer_enabled(enabled: bool, force_no: bool = False):
        if force_no:
            try:
                ...
                pass
            # Also update the info label visibility when forcing No
            try:
                on_item_randomizer_change()
            except Exception:
                pass
    ...
    # Info box (ALWAYS visible, directly under the title)
    item_info_box = ctk.CTkFrame(
        sec_item,
        corner_radius=8,
        fg_color="#2d2d2d",
    )
    item_info_box.pack(fill="x", padx=12, pady=(2, 10))

    item_info_label = ctk.CTkLabel(
        item_info_box,
        text=(
            "⚠️  Note: The game only supports character randomizer.\n"
            "Items are randomized only in the STATS – not in-game."
        ),
        font=ctk.CTkFont(size=12),
        text_color="gray80",
        justify="left",
    )
    item_info_label.pack(anchor="w", padx=10, pady=8)

    item_var = ctk.StringVar(value="No")

    # Radio buttons (under the info)
    radio_frame = ctk.CTkFrame(sec_item, fg_color="transparent")
    radio_frame.pack(anchor="w", padx=12, pady=(0, 6))

    item_yes = ctk.CTkRadioButton(
        radio_frame,
        text="Yes",
        variable=item_var,
        value="Yes",
    ...
    item_yes.pack(side="left", padx=(0, 12))
    item_no.pack(side="left", padx=(0, 12))

    # Dummy function so Target Mode / Seed Lock calls remain safe
    def on_item_randomizer_change():
        pass

    # Continue with Random Character section
    ...
    def _update_seed_lock(*_):
        ...
        locked = bool(seed_text)

        if locked:
            m = seed_code_extended_pattern.match(seed_text)
            if m:
                try:
                    rooms = int(m.group("R"))
                ...
                if rooms is not None:
                    try:
                        val = max(0, min(rooms, total_rooms))
                        level_slider.set(val)
                        level_val_lbl.configure(text=f"Selected: {val} / {total_rooms}")
                    except Exception:
                        pass

                if bosses is not None:
                    try:
                        val = max(0, min(bosses, total_bosses))
                        boss_slider.set(val)
                        boss_val_lbl.configure(text=f"Selected: {val} / {total_bosses}")
                    except Exception:
                        pass

                if c_flag is not None:
                    if c_flag == 0:
                        random_char_var.set(False)
                        random_char_stage_var.set(False)
                    elif c_flag == 1:
                        random_char_var.set(True)
                        random_char_stage_var.set(False)
                    else:
                        random_char_var.set(True)
                        random_char_stage_var.set(True)
                ...
                def _enabled_from_mask(order, mask_int: int):
                    enabled = set()
                ...
                if er:
                    try:
                        er_mask = int(er, 16)
                        enabled_rooms = _enabled_from_mask(room_order, er_mask)
                        for name, var in room_vars.items():
                            var.set(name in enabled_rooms)
                    except Exception:
                        pass
                ...
                if eb:
                    try:
                        eb_mask = int(eb, 16)
                        enabled_bosses = _enabled_from_mask(boss_order, eb_mask)
                        for name, var in boss_vars.items():
                            var.set(name in enabled_bosses)
                    except Exception:
                        pass
        ...
        if locked:
            for chk in boss_checks.values():
                try:
                    chk.configure(state="disabled")
                except Exception:
                    pass
            for chk in room_checks.values():
                try:
                    chk.configure(state="disabled")
                except Exception:
                    pass
    ...
    def on_start():
        ...
        disabled_from_code = False

        if seed_text:
            m_ext = extended_pattern.match(seed_text)
            if m_ext:
                try:
                    rooms = int(m_ext.group("R"))
                    bosses = int(m_ext.group("B"))
                ...
                def _decode_mask(order, mask_int: int):
                    enabled = []
                    disabled = []
                    for idx, name in enumerate(order):
                        if mask_int & (1 << idx):
                ...
                try:
                    if er:
                        room_mask = int(er, 16)
                        _, disabled_rooms = _decode_mask(room_order, room_mask)
                        cfg.disabled_rooms = set(disabled_rooms)
                    if eb:
                        boss_mask = int(eb, 16)
                        _, disabled_bosses = _decode_mask(boss_order, boss_mask)
                        cfg.disabled_bosses = set(disabled_bosses)
                    disabled_from_code = bool(er or eb)
                except Exception:
                    disabled_from_code = False

            else:
                m = legacy_pattern.match(seed_text)
                if m:
                    try:
                        rooms = int(m.group("R"))
                        bosses = int(m.group("B"))
