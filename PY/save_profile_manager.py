...
def _load_profiles() -> Dict[str, List[str]]:
    if not os.path.exists(SAVE_PROFILES_JSON):
        raise FileNotFoundError(
            f"save_profiles.json not found at {SAVE_PROFILES_JSON}"
        )
    ...
    for name, obj in raw.items():
        if isinstance(obj, dict) and "content" in obj and isinstance(
            obj["content"], list
        ):
            profiles[name] = [str(line) for line in obj["content"]]
        else:
            raise ValueError(
                f"Invalid profile format for '{name}' in save_profiles.json"
            )
...
def write_all_saves_from_profiles(tag: str = "init") -> None:
    ...
    if "SaveFile1" in profiles:
        try:
            _write_single_profile(
                "SaveFile1",
                profiles["SaveFile1"],
                SAVE_PATHS["SaveFile1"],
            )
        except Exception as e:
            log(f"⚠️ [save_profile_manager] Failed to write SaveFile1: {e}")
    else:
        log(
            "⚠️ [save_profile_manager] Profile 'SaveFile1' missing "
            "in save_profiles.json"
        )
        ...
        for logical_name in ("SaveFile2", "SaveFile3"):
            try:
                _write_single_profile(
                    logical_name,
                    profiles["SaveFile2"],
                    SAVE_PATHS[logical_name],
                )
            except Exception as e:
                log(
                    f"⚠️ [save_profile_manager] Failed to write "
                    f"{logical_name}: {e}"
                )
    else:
        log(
            "⚠️ [save_profile_manager] Profile 'SaveFile2' missing "
            "(used for SaveFile2 & SaveFile3)"
        )
    ...
    if "onlineLicense" in profiles:
        try:
            _write_single_profile(
                "onlineLicense",
                profiles["onlineLicense"],
                SAVE_PATHS["onlineLicense"],
            )
        except Exception as e:
            log(
                f"⚠️ [save_profile_manager] Failed to write "
        ...
        log(
            "⚠️ [save_profile_manager] Profile 'onlineLicense' missing "
            "in save_profiles.json"
        )
