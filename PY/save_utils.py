...
def _short_stack(max_lines=4) -> str:
    ...
    try:
        stack = "".join(
            traceback.format_stack(limit=max_lines + 2)[-max_lines - 1 : -1]
        ).strip()
        return stack
    except Exception:
        return ""
...
def restore_if_save_shrunk(
    ...
    try:
        ...
        for fn in os.listdir(SNAPSHOT_DIR):
            if (
                fn.startswith("route_backup_")
                or fn.startswith("pre_transition_")
                or fn.startswith("audit_post_transition_")
            ):
    ...
    while time.time() - t0 < window_s:
        ...
        if (
            now_sz == 0
            or now_sz < min_expected
            or (
                before_size
                and now_sz < max(48, int(before_size * 0.5))
            )
        ):
            if best_snapshot and os.path.exists(best_snapshot):
                try:
                    ...
                    text, was_encrypted = smart_read(save_enc)
                    smart_write(save_enc, plain, was_encrypted)

                    mark_last_writer(
                        "auto_restore_after_shrink",
                        {
                    ...
                    log(
                        f"🛡️ Save shrunk (now {now_sz}B). "
                        f"Restored from snapshot: {os.path.basename(best_snapshot)}"
                    )
                    mirror_plain_for_tracker()
                    return True
                except Exception as e:
