# ======================================================
# loading_overlay.py (Tkinter)
# Centered "Loading next stage..." overlay  +  optional BLACKOUT
# - Frameless, always-on-top
# - Ultra-short fade-in/out (ca. 120ms)
# - Text & optional logo centered über Game-Fenster
# - Blocking API:  show_loading_overlay(...)
# - Async API:     begin_loading_overlay(...), end_loading_overlay()
# ======================================================
...
try:
    ...
    overlays_enabled = True  # Default: Overlay erlaubt
...
def _find_game_window_rect():
    """
    Return (left, top, width, height) for the game window, else None.
    Strategy: exact match on window_title -> contains match on window_title_variants.
    """
    try:
        if gw is None:
            return None
...
def _center_rect_over(width, height):
    """
    Liefert (x, y, w, h) für ein zentriertes Overlay-Fenster
    über dem Game-Window, oder zentriert auf dem Hauptbildschirm.
    """
...
def _full_game_rect():
    """
    Liefert (x, y, w, h) für einen Vollbild-Overlay-Bereich
    über dem Game-Window oder ganzen Screen.
    """
    rect = _find_game_window_rect()
    if rect:
        l, t, w, h = rect
...
def _overlay_loop(text, step_info, blackout, max_duration_s, route_code=None):
    ...
    Schließt, wenn:
      - _overlay_stop_event gesetzt wird
      - oder external_stop_event (Stop vom Randomizer) gesetzt wird
      - oder max_duration_s abgelaufen ist
    """
    ...
    # Optional: Randomizer-Logo laden
    logo_img = None
    try:
        project_root = os.path.dirname(ini_folder)  # z.B. C:\Boshy Randomizer
    ...
    # Layout: alles in einem Frame, der komplett expandiert
    ...
    try:
        ...
        for i in range(steps + 1):
            ...
            root.update()
            time.sleep(0.02)  # ~20 ms -> ~120ms total
    except Exception:
        pass
...
def show_loading_overlay(stop_event=None, duration=0.3,
    ...
    Für deinen Randomizer reicht ein sehr kurzer Wert (0.3s).
    """
...
def begin_loading_overlay(stop_event=None, text="Loading next stage...",
    ...
    Nicht-blockierende Variante:
      ...
      - Overlay bleibt an, bis end_loading_overlay() aufgerufen wird
        oder stop_event gesetzt wird oder max_duration_s abläuft.
      - max_duration_s bewusst klein gehalten, weil dein Trigger sehr schnell ist.
    """
    if not overlays_enabled:
        return
    ...
    with _overlay_lock:
        # Wenn bereits ein Overlay läuft, lasse es weiterlaufen
...
def end_loading_overlay():
    """
    Löst das Beenden des Tkinter-Overlays aus.
    Bleibt no-op, wenn kein Overlay aktiv ist.
    """
    if not overlays_enabled:
        return
    ...
    # Optional: kurz warten, bis Thread fertig ist
    try:
