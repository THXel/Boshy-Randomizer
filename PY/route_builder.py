...
def apply_history_filter(
    ...
    if not use_history:
        return enabled_rooms, enabled_bosses
...
def build_random_route(cfg, pos_data=None, mode: str = "normal"):
    ...
    # Seed detection BEFORE history filter
    seed_raw = getattr(cfg, "route_seed", None)
    seed_int: int | None = None
    use_seed = False

    if seed_raw not in (None, "", False):
        try:
            seed_int = int(seed_raw)
            use_seed = True
        except Exception:
            log(f"⚠️ route_seed is not a valid int ({seed_raw!r}) – using normal RNG.")
            seed_int = None
            use_seed = False
    ...
    if not use_seed:
        ...
        log("🔒 Seed-lock active – route history is ignored for this run.")

    route: list[str] = []
    enabled_room_set = set(enabled_rooms)
    enabled_boss_set = set(enabled_bosses)

    if use_seed and seed_int is not None:
